...
| `EW` | Select one or many placed doors/windows → deletes each and closes the wall back up. Works with LINE walls and open/closed LWPOLYLINE walls. With AKD WallTool loaded, the same `EW` also erases walls, and a selection may mix walls with doors/windows. If a column is picked, `EW` delegates to AKDColumn's `EC` (which handles group deletion and wall rejoin via its own xdata) — re-pick the column when prompted. |
...
- On a WallTool wall, `AD` / `AW` / `ACW` place the object and let WallTool cut the opening; `EW`, `CW`, `VX` and `RH` let WallTool close or re-cut it. On ordinary walls these commands behave as before.
...
- `EW` is shared: with both loaded, WallTool's `EW` erases walls and hands any doors/windows in the selection to WinDoor (hook `*wt:erase-fns*`), in one undo step. WinDoor defines `EW` only when WallTool is absent. Renumbering stays `WRN`, since WallTool owns `WWR`.
