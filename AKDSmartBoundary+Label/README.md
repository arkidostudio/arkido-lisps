...
- **`SB`** — auto mode. Pick an internal point, SB detects openings and bridges them, then runs `BOUNDARY`.
  ...
  - `Hatch` option (or type `H`) — toggle auto-hatch mode. When ON, each new boundary gets hatched immediately using the current default pattern (`HPNAME`/`HPSCALE`/`HPANG`) with a random index color at 25% opacity. The hatch is placed on the boundary output layer and sent to the back.
