...
;;; wall  = (ename p1 p2 thickness position)   position: "CENTER" | "LEFT" | "RIGHT"
...
;; Guess thickness/position of an unregistered master from parallel A-WALL
;; lines overlapping its span. nil when no consistent face pair exists
;; (plain axis, or ambiguous geometry).
(defun wt:recon (m faces / p1 p2 u l d sa sb pmin nmax zero)
  ...
  (cond
    ...
    ((and zero pmin (or (not nmax) (<= pmin (- nmax)))) (list (car m) p1 p2 pmin "LEFT"))
    ((and zero nmax) (list (car m) p1 p2 (- nmax) "RIGHT"))))
...
;; Add walls on master segments ((a b) ...) with current settings as ONE
;; transaction. Returns the (created erased modified) record, or nil if rejected.
(defun wt:walls-add (segs / net new en rec bad)
  (setq net (wt:net-scan))
  (foreach s segs
    (cond ((wt:peq (car s) (cadr s)) (setq bad "\nZero-length wall ignored."))
          ((and (setq en (wt:master-at (car s) (cadr s) (car net)))
  ...
  (if bad
    ...
    (progn
      (wt:pend-begin)
      (foreach s segs
        (if (setq en (wt:pend-make (wt:mk-line (car s) (cadr s) (wt:cfg "AXIS_LAYER"))))
          (setq new (cons (list en (car s) (cadr s) *wt:thk* *wt:pos*) new))))
      (setq new (reverse new))
      (setq new (wt:rebuild new nil))
      (foreach w new (wt:reg-add (car w) (wt:w-thk w) (wt:w-pos w)))
...
(defun c:WW (/ *error* p0 p q pts hist r done)
  (setq *error* wt:error)
  (wt:begin)
  (wt:layer "AXIS")
  (wt:layer "WALL")
...
;; A source on another layer is moved to X-AXIS; a source duplicating an
;; existing X-AXIS line is erased and that line becomes the master.
(defun wt:xw-convert (sel / net axis new skipped d a b ex)
  ...
  (foreach en sel
    ...
    (cond
      ((or (wt:peq a b)
           ...
           (wt:master-at a b new))
       (setq skipped (1+ skipped)))
      ((setq ex (wt:master-at a b (car net)))
       (if (wt:wall-from-master ex (cadr net))
         (setq skipped (1+ skipped))
         (progn
           (if (not (eq (car ex) en)) (wt:pend-erase en))
           (setq new (cons (list (car ex) (cadr ex) (caddr ex) *wt:thk* *wt:pos*) new)))))
      (t
       (wt:pend-modify (wt:bylayer d axis))
       (setq new (cons (list en a b *wt:thk* *wt:pos*) new)))))
  (setq new (reverse new))
  (setq ex (length new))
  (if new (setq new (wt:rebuild new nil)))
  (foreach w new (wt:reg-add (car w) (wt:w-thk w) (wt:w-pos w)))
  (setq *wt:pending* nil)
  (if (> skipped 0)
